...
class VIPERUnifiedTradingJob:
    ...
    def _load_configuration(self):
        ...
        try:
            # Trading parameters - FIXED FOR 2% RISK MANAGEMENT
            self.trading_config = {
                'max_positions_total': int(os.getenv('MAX_TOTAL_POSITIONS', '10')),
                ...
                'min_volume_threshold': float(os.getenv('MIN_VOLUME_THRESHOLD', '10000')),  # $10K daily volume (more inclusive for testing)
                'min_leverage_required': int(os.getenv('MIN_LEVERAGE_REQUIRED', '1')),  # 1x leverage minimum (for all active pairs)
    ...
    def _calculate_viper_scores(self, opportunity: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            return {'volume': 0, 'price': 0, 'leverage': 0, 'technical': 0, 'risk': 0}
