...
class ViperAsyncTrader:
    ...
    def __init__(self):
        ...
        # Initialize Data Streaming with Enhanced Support
        if OPTIMIZED_DATA_AVAILABLE:
            self.optimized_data_streamer = None
            self.use_optimized_data = True
        ...
        if ENHANCED_TECHNICAL_AVAILABLE:
            ...
            self.enhanced_technical = None  # Will be set from integrator
            self.use_enhanced_technical = True

            # Try to get enhanced technical optimizer from integrator
            try:
                integrator = get_integrator()
            ...
            if not self.use_enhanced_technical:
                # Fallback to basic trend detector
                self._initialize_basic_trend_detector()
        else:
            # Use basic trend detector
            self.use_enhanced_technical = False
            self._initialize_basic_trend_detector()
    ...
    def calculate_position_size(self, price: float, balance: float, leverage: int = 50):
        ...
        try:
            ...
            logger.info(f"# Target Position Sizing: Balance=${balance:.2f}, Risk={risk_per_trade*100}% (${risk_amount:.2f}), "
...
import secrets
    ...
    async def calculate_advanced_trend_score(self, symbol: str) -> Tuple[float, Optional[TrendDirection]]:
        ...
        try:
            if self.use_enhanced_technical and self.enhanced_technical:
                # Use enhanced technical optimizer
                return await self._calculate_enhanced_trend_score(symbol)
            else:
                # Fallback to basic trend detector
    ...
    async def _calculate_enhanced_trend_score(self, symbol: str) -> Tuple[float, Optional[EnhancedTrendDirection]]:
        ...
        try:
            ...
            # Adjust score based on confidence and confluence
            confidence_multiplier = trend_analysis.confidence
            confluence_bonus = trend_analysis.confluence_score * 10  # Up to 10 points bonus
